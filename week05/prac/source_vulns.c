// codereview.stackexchange.com/questions/210069/a-simple-password-manager-in-c
...
static void usage(void) { printf("USAGE: pw CMD KEY\n"); }
...
static char *get_store_dir(void) {
  ...
  /* integer overflow can build a large required string, and small min_len
   * string */
...
static char *get_passfile(const char *dir, const char *key) {
  ...
  /* VULN - allows to build format string that can be triggered if path is too
   * long */
...
static char *decrypt_from_file(const char *path, size_t *len, char *pass) {
  ...
  /* no check to see if pass is successfull */
  pass = malloc(strlen(cc.data[0]));
  /* attempt to write to NULL will segfault */
...
static int encrypt_to_file(const char *path, char *buf, int len) {
  ...
  if (gpgme_op_encrypt(cc.ctx, cc.keylist, GPGME_ENCRYPT_ALWAYS_TRUST,
    ...
    gpgme_data_release(cc.data[0]);
    gpgme_data_release(cc.data[1]);
    free(buf); /* Double Free triggered */
    return 1;
  }
...
static int init_crypto(void) {
  ...
  if (key == NULL) {
    ...
    else { /* dangling else means gpgme_get_key is never checked!*/
...
static int insert_entry(const char *keyfile) {
  ...
  /* input_len is cast to type size_t, but encrypt_to_file uses it
     as an int -> integer overflow */
  char *input_buffer;
  size_t input_len = get_console_input(input_buffer);
  /* BUG - What if password length is 0??? */
  if (input_len < 0) {
...
static int get_entry(const char *keyfile) {
  ...
  /* timing attack possible against password */
